(() => {
  ...
  function renderQuestionReview(current) {
    ...
    const rows = items.map((item) => {
      ...
      return `<article class="results-review-item ${correct ? 'is-correct' : hasAnswer ? 'is-wrong' : ''}" data-section="${escapeHtml(item.question.section)}" data-result="${correct ? 'correct' : hasAnswer ? 'incorrect' : 'unanswered'}"><div class="results-review-meta"><span>${escapeHtml(item.module.label)} · Question ${item.index + 1}</span><span class="results-review-status ${correct ? 'correct' : 'wrong'}">${correct ? 'Correct' : hasAnswer ? 'Incorrect' : 'Unanswered'}</span></div><div class="results-review-prompt">${escapeHtml(item.question.prompt)}</div><div class="results-review-answer"><strong>Your answer:</strong> ${escapeHtml(selected)} <span>·</span> <strong>Correct:</strong> ${escapeHtml(correctText)}</div><details class="results-review-details"><summary>Review answer</summary><div class="results-review-explanation">${escapeHtml(item.question.explanation)}</div></details></article>`;
    }).join('');
