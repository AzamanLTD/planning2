(() => {
  ...
  function download() {
    ...
    link.href = url; link.download = `azaman-sat-practice-report-${name}.html`; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  function install() {
    ...
    const button = document.createElement('button'); button.id = 'downloadPracticeReport'; button.type = 'button'; button.className = 'btn'; button.textContent = 'Download practice report'; button.addEventListener('click', download);
