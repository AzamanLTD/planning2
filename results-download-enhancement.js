(() => {
  ...
  function readState() { try { const value = JSON.parse(localStorage.getItem(KEY) || 'null'); return value && value.v === 3 ? value : null; } catch (_) { return null; } }
  ...
  function buildReport(current) {
    ...
    const modules = MODULES.map((module) => {
      const bank = bankFor(module, current); let moduleCorrect = 0, moduleAnswered = 0;
      bank.forEach((question, index) => { const answer = current.answers?.[`${module.id}-${index}`]; if (answer === undefined || String(answer).trim() === '') return; moduleAnswered += 1; if (equivalent(answer, question.answer)) moduleCorrect += 1; });
      correct += moduleCorrect; answered += moduleAnswered; total += bank.length;
  ...
  function download() { const current = readState(); if (!current || current.screen !== 'finish') return; const html = buildReport(current); const blob = new Blob([html], { type: 'text/html;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); const name = String(current.student || 'student').trim().replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase() || 'student'; link.href = url; link.download = `azaman-sat-practice-report-${name}.html`; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
  function install() { if (!document.querySelector('.kicker') || ![...document.querySelectorAll('.kicker')].some((el) => el.textContent.trim() === 'Practice complete')) return; const card = [...document.querySelectorAll('.kicker')].find((el) => el.textContent.trim() === 'Practice complete')?.closest('.card'); if (!card || card.querySelector('#downloadPracticeReport')) return; const button = document.createElement('button'); button.id = 'downloadPracticeReport'; button.type = 'button'; button.className = 'btn'; button.textContent = 'Download practice report'; button.addEventListener('click', download); (card.querySelector('.btn-row') || card).prepend(button); }
