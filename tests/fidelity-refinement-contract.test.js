...
for (const selector of ['.bb-final-device-card','.bb-final-device-row','.bb-final-at-card']) if (!finalCss.includes(selector)) throw new Error(`missing final calibration selector: ${selector}`);
...
if (!sw.includes("const CACHE_NAME='azaman-bluebook-v16'")) throw new Error('offline cache version does not match current precache generation');
