...
if (fs.existsSync(path.join(root, 'bluebook-final-visual-flow.css'))) throw new Error('obsolete duplicate visual flow layer remains');
...
for (const text of ['Section I, Module 1: Reading and Writing','Section II, Module 1: Math','multiple-choice with four answer choices','reference sheet, calculator, and these directions']) if (!examJs.includes(text)) throw new Error(`missing official exam wording: ${text}`);
if (pixelCss.includes('content:"100%"')) throw new Error('pixel layer contains non-Bluebook percentage meter');
