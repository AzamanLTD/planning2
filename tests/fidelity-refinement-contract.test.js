...
for (const text of ['Section 1: Reading and Writing','Section 2: Math','multiple-choice questions','You can move back and forth between questions until time expires.','Once the next module begins, you cannot return to these questions.','state.mi === 0 && state.step === 10','state.endAt = Date.now() + 32 * 60 * 1000']) if (!examJs.includes(text)) throw new Error(`missing official exam wording/timer behavior: ${text}`);
